...
class AirCanadaFlightsManager(IReservationManager):
    ...
    def search(self, request: FlightRequest):
        flights_external = AirCanadaOnlineAPI.get_flights(request.from_loc, request.datetime_from, request.to_loc, request.datetime_to, request.adults, request.children)
